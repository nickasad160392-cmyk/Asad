...
export default function AbsenPage() {
  ...
  const startDetection = useCallback(
    async (faceapi: any) => {
      ...
      const detectLoop = async () => {
        ...
        else if (status !== "ready") setStatus("detecting");
  ...
  const handleStart = useCallback(async () => {
    if (hasCheckedOut) {
      toast.info("Absensi hari ini sudah selesai.");
      return;
    }
    ...
    if (!coords) {
      toast.warning("Lokasi tidak terdeteksi. Koordinat akan kosong.");
    }
    setGps(coords);

    if (faceapi) {
      await startDetection(faceapi);
    } else {
      setStatus("ready");
    }
  ...
  const captureAndSubmit = useCallback(async () => {
    ...
    mutation.mutate(body, {
      onSuccess: () => {
        ...
        setStatus("done");
        const msg = hasCheckedIn ? "Absen keluar berhasil! 🎉" : "Absen masuk berhasil! ✅";
        toast.success(msg);
        setTimeout(() => router.replace("/dashboard"), 2000);
      },
      onError: (err: any) => {
        const msg = err?.response?.data?.error || err?.data?.error || "Gagal menyimpan absensi.";
        toast.error(msg);
        setStatus("ready");
  ...
  }, [status, gps, hasCheckedIn, checkIn, checkOut, queryClient, router, stopCamera, handleStart]);
      ...
      <div className="flex-1 px-5 pt-6 flex flex-col items-center">
        ...
        {(status === "detecting" || status === "ready" || status === "capturing" || status === "submitting") && (
          <div className="w-full flex flex-col items-center">
            ...
            <button
              onClick={() => { stopCamera(); setStatus("idle"); setFaceDetected(false); }}
