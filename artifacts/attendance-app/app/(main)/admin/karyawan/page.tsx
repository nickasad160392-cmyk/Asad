...
import { ArrowLeft, Search, X, Loader2, Shield, User, UserCog, ChevronDown } from "lucide-react";
...
export default function AdminKaryawanPage() {
  ...
  const handleSave = () => {
    ...
    setIsSaving(true);
    updateUser.mutate(
      { id: selected.id, data: { jabatan: jabatan || undefined, isActive } },
      {
        onSuccess: () => {
  ...
  const handleResetPassword = () => {
    ...
    resetPassword.mutate(
      { id: selected.id, data: { newPassword } },
      {
        onSuccess: () => {
        ...
        onError: (err: any) => {
          toast.error(err?.response?.data?.error || "Gagal mereset kata sandi");
          setIsResetting(false);
        },
      },
    );
          ...
          <div className="bg-white w-full max-w-[430px] rounded-t-3xl px-5 pt-5 pb-8 max-h-[85vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between bg-gray-50 rounded-xl px-4 py-3">
                ...
                >
                  <div className={`w-5 h-5 rounded-full bg-white shadow absolute top-0.5 transition-all ${isActive ? "left-6.5 translate-x-0.5" : "left-0.5"}`} />
                </button>
              </div>
