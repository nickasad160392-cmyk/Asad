...
function exportCSV(data: any[], cycleLabel: string) {
  ...
  const rows = data.map((r) => [
    r.userName, r.userEmployeeId || "", r.userJabatan || "",
    r.date, r.status,
...
export default function AdminRekapPage() {
  ...
  const { data: records, isLoading } = useGetAdminAttendance(
    { cycleStart: selectedCycle },
    { query: { queryKey: getGetAdminAttendanceQueryKey({ cycleStart: selectedCycle }) } },
  );
      ...
      <div className="bg-[#FACC15] px-5 pt-12 pb-8 rounded-b-[40px]">
        <div className="flex items-center justify-between mb-4">
          ...
          {records && records.length > 0 && (
            <button
              onClick={() => exportCSV(records, cycleLabel)}
