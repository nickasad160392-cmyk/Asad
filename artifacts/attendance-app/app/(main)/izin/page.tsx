...
export default function IzinPage() {
  ...
  const { data: requests, isLoading } = useGetMyLeaveRequests(
    {},
    { query: { queryKey: getGetMyLeaveRequestsQueryKey({}) } },
  );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    submitLeave.mutate(
      { data: { type, startDate, endDate, reason } },
      {
        ...
        onError: (err: any) => {
          const msg = err?.response?.data?.error || "Gagal mengajukan izin.";
          toast.error(msg);
          setIsSubmitting(false);
        },
      },
                    ...
                    {r.adminNote && (
                      <div className="mt-2 bg-gray-50 rounded-lg px-3 py-2">
                        <p className="text-xs text-[#8C8573]">
                          <span className="font-semibold">Catatan Admin:</span> {r.adminNote}
                        </p>
                      </div>
                    )}
                  </div>
