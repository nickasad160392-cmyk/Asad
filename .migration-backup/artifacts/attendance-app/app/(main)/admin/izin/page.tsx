...
import {
  useGetAdminLeaveRequests,
  getGetAdminLeaveRequestsQueryKey,
  useApproveLeave,
  useRejectLeave,
...
export default function AdminIzinPage() {
  ...
  const { data: leaves, isLoading } = useGetAdminLeaveRequests(
    { status: filterStatus },
    { query: { queryKey: getGetAdminLeaveRequestsQueryKey({ status: filterStatus }) } },
  );
  ...
  const handleApprove = (id: number) => {
    setProcessingId(id);
    approveLeave.mutate(
      { id, data: { adminNote: undefined } },
      {
        onSuccess: () => {
          queryClient.invalidateQueries({ queryKey: getGetAdminLeaveRequestsQueryKey({ status: filterStatus }) });
          toast.success("Pengajuan disetujui");
          setProcessingId(null);
        },
        onError: () => { toast.error("Gagal menyetujui"); setProcessingId(null); },
      },
    );
  };
  ...
  const handleReject = () => {
    ...
    rejectLeave.mutate(
      { id: rejectId, data: { adminNote: adminNote || undefined } },
      {
        onSuccess: () => {
          queryClient.invalidateQueries({ queryKey: getGetAdminLeaveRequestsQueryKey({ status: filterStatus }) });
        ...
        onError: () => { toast.error("Gagal menolak"); setProcessingId(null); },
      },
    );
  };
              ...
              return (
                <div key={leave.id} className="bg-white rounded-2xl p-4 shadow-sm">
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    <span className="bg-[#FACC15]/20 text-[#4A4435] text-[10px] font-bold px-2 py-0.5 rounded-full">
                      {typeLabel}
                    </span>
                    <span className="text-xs text-[#8C8573]">
                  ...
                  {isPending && (
                    <div className="flex gap-2 mt-3">
                      ...
                      <button
                        onClick={() => { setRejectId(leave.id); setAdminNote(""); }}
                        disabled={!!processingId}
