...
export default function RiwayatPage() {
  ...
  const { data: history, isLoading: loadingHistory } = useGetAttendanceHistory(
    { cycleStart: selectedCycle },
    { query: { queryKey: getGetAttendanceHistoryQueryKey({ cycleStart: selectedCycle }) } },
  );

  const { data: summary, isLoading: loadingSummary } = useGetCycleSummary(
    { cycleStart: selectedCycle },
    { query: { queryKey: getGetCycleSummaryQueryKey({ cycleStart: selectedCycle }) } },
  );
                ...
                <div key={r.id} className="bg-white rounded-2xl p-4 shadow-sm">
                  ...
                  {r.checkInSelfie && (
                    <details className="mt-2">
                      ...
                      <img
                        src={`data:image/jpeg;base64,${r.checkInSelfie}`}
                        alt="Selfie"
                        className="w-16 h-16 rounded-lg object-cover mt-1 scale-x-[-1]"
                      />
                    </details>
                  )}
                </div>
