...
export default function DashboardPage() {
  ...
  const { data: cycle, isLoading: loadingCycle } = useGetCycleSummary(
    { cycleStart },
    { query: { queryKey: getGetCycleSummaryQueryKey({ cycleStart }) } },
  );
  ...
  const metrics = [
    {
      label: "JAM KERJA",
      value: cycle ? Math.floor((cycle.totalWorkMinutes ?? 0) / 60) : "--",
      unit: "jam",
      color: "text-[#4A4435]",
    },
    {
      label: "LEMBUR",
      value: cycle ? Math.floor((cycle.totalOvertimeMinutes ?? 0) / 60) : "--",
      unit: "jam",
      color: "text-[#FACC15]",
    },
    {
      label: "TERLAMBAT",
      value: cycle ? (cycle.totalLatenessMinutes ?? 0) : "--",
      unit: "mnt",
      color: "text-[#E57373]",
    },
    {
      label: "IZIN",
      value: cycle ? (cycle.permitDays ?? 0) : "--",
      unit: "hari",
      color: "text-[#64B5F6]",
    },
  ];
      ...
      <div className="bg-[#FACC15] px-5 pt-12 pb-10 rounded-b-[48px] relative z-10 shadow-md">
        <div className="flex items-start justify-between mb-5">
          ...
          <div className="flex items-center gap-2">
            <Link href="/admin" className={`${user?.role === "admin" || user?.role === "hr" ? "" : "hidden"} w-8 h-8 rounded-full bg-[#4A4435]/10 flex items-center justify-center`}>
